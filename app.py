...
def salvar_dados_github(df_salvar, sha=None):
    ...
    return resposta # Agora retorna a resposta completa para vermos o erro
...
if is_admin:
    ...
    with st.sidebar.expander("📥 Subir Planilha Diária", expanded=True):
        uploaded_file = st.file_uploader("Arraste o relatório de entregas", type=["xlsx", "csv"])
        if uploaded_file:
            if st.button("💾 Substituir Dados no Painel"):
                # Verifica se o Token existe antes de tentar
                if not GITHUB_TOKEN:
                    ...
                    with st.spinner("Atualizando painel do gerente..."):
                        try:
                            ...
                            if resposta_github.status_code in [200, 201]:
                                ...
                                st.rerun()
                            elif resposta_github.status_code == 401:
                                st.error("❌ ERRO 401: O GitHub recusou o seu Token. Ele pode estar errado ou expirado.")
                            elif resposta_github.status_code == 404:
                                st.error("❌ ERRO 404: O GitHub não achou o repositório. O Token não tem a permissão 'repo' marcada.")
                            else:
...
if df_banco is not None and not df_banco.empty:
    ...
    def classificar_status(row):
        ...
        if entrega not in ['', 'nan', 'None']: return '🟢 Entregue'
